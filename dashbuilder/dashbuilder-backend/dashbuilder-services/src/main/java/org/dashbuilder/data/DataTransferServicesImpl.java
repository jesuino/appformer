...
public class DataTransferServicesImpl implements DataTransferServices {
    ...
    private FileSystem dataTransferFS;
    ...
    public DataTransferServicesImpl(
            ...
            final @Named("dataTransferFS") FileSystem dataTransferFS,
        ...
        this.dataTransferFS = dataTransferFS;
    ...
    public String doExport() throws Exception {
    	String zipLocation = new StringBuilder()
    			...
    			.append(DataTransferServices.FILE_UPLOAD_NAME)
    			.toString();
        ...
        moveZipToFileSystem(zipLocation, dataTransferFS);

    	String exportPath = new StringBuilder()
    			.append(SpacesAPI.Scheme.GIT)
    			.append("://")
    			.append(dataTransferFS.getName())
    			.append(File.separator)
    			.append(DataTransferServices.FILE_UPLOAD_NAME)
    			.toString();
    ...
    public List<String> doImport() throws Exception {    	
    	...
    	Path root = dataTransferFS.getRootDirectories().iterator().next();    	

    	Files.walkFileTree(root, new SimpleFileVisitor<Path>() {
    		@Override
    		public FileVisitResult visitFile(Path path, BasicFileAttributes attrs) throws IOException {
    			if (!path.getFileName().toString().equalsIgnoreCase(DataTransferServices.FILE_UPLOAD_NAME)) {
    ...
    private void fireEvent(File newFile, String tempPath, URI uri, String newFilePath) throws Exception {
        ...
        if (filePath.contains(tempPath + datasetsFS.getName()) && newFilePath.endsWith(".dset")) {
        	fireDatasetEvent(uri, newFilePath);
			
       	} else if (filePath.contains(tempPath + perspectivesFS.getName()) && newFilePath.endsWith(".plugin")) {
       		firePerspectiveEvent(newFile, uri, newFilePath);
      		
       	} else if (filePath.contains(tempPath + navigationFS.getName())) {
       		fireNavigationEvent();
       	}
    }
    ...
    private void fireNavigationEvent() {
    	navTreeChangedEvent.fire(new NavTreeChangedEvent(navTreeStorage.loadNavTree()));
    }
