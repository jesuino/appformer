...
//    }
    ...
    public void setAnimationEnabled(boolean animationEnabled) {
        settings.put(getSettingPath(DisplayerAttributeDef.ANIMATION_ENABLED), Boolean.toString(animationEnabled));
    }
    
    public boolean isAnimationEnabled() {
        return parseBoolean(settings.get(getSettingPath(DisplayerAttributeDef.ANIMATION_ENABLED)), false);
    }
    
    
    public void setAutoStart(boolean autoStart) {
        settings.put(getSettingPath(DisplayerAttributeDef.AUTO_START), Boolean.toString(autoStart));
    }
    
    public boolean isAutoStart() {
        return parseBoolean(settings.get(getSettingPath(DisplayerAttributeDef.AUTO_START)), true);
    }
    
    public void setDelay(long delay) {
        settings.put(getSettingPath(DisplayerAttributeDef.DELAY), Long.toString(delay));
    }
    
    public long getDelay() {
        return parseLong(settings.get(getSettingPath(DisplayerAttributeDef.DELAY)), 0);
    }
    
    public void setTransitionDuration(long duration) {
        settings.put(getSettingPath(DisplayerAttributeDef.TRANSITION_DURATION), Long.toString(duration));
    }
    
    public long getTransitionDuration() {
        return parseLong(settings.get(getSettingPath(DisplayerAttributeDef.TRANSITION_DURATION)), 0);
    }
    
    public void setAnimationTarget(AnimationTarget animationTarget) {
        settings.put(getSettingPath(DisplayerAttributeDef.ANIMATION_TARGET), animationTarget.toString());
    }
    
    public AnimationTarget getAnimationTarget() {
        return AnimationTarget.from(settings.get(getSettingPath(DisplayerAttributeDef.ANIMATION_TARGET)));
    }
